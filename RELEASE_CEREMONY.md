...
- [ ] `bash scripts/verify-migration-equivalence.sh --self-check` passes both polarities, run **before** the equivalence check below. Squashes are months apart and the script can rot in between — a `pg_dump` rendering change, a normalisation that stops normalising — so what the next criterion rests on is that the tool works *now*, not that it worked whenever someone last edited it
