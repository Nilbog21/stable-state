...
- A patch's `CHANGELOG.md` entry is **not** its own `## ` section (#1589). It is a `**vN.0.x — {Month YYYY}.** {prose}` paragraph, newest first, under a `### Later updates` heading at the top of the current major's section — created there if that heading doesn't exist yet. `/changelog`'s contents list names `##` headings only, and both version parsers read the newest release out of that block, so a top-level patch heading breaks all three at once
