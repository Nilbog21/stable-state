...
- **Cross-identity flows** — needs a genuinely separate real person: invite/claim by someone else, cross-user notification delivery, or a self-write whose author is a different real *account*. Since #1563 `change-user.sh` moves `profiles.user_id` too, a self-write *as* an impersonated persona is testable locally and no longer clears this bar on its own — what remains here is the case where the writing uid belongs to someone else, not your own uid wearing another profile row. A *fresh or unauthenticated* session does **not** clear this bar either; incognito covers that locally, so those stay in PRE
...
> Release-4 is the first deploy that needs any of this: `/demo` (#505) and its reaper cron (#506, moved to a daily sweep by #1438) both ship in it, so none of these variables exist yet on a project that has only ever run an earlier release. **Skip this whole block if the Vercel project's production environment already lists `DEMO_USER_EMAIL`, `DEMO_USER_PASSWORD` and `CRON_SECRET`** — there is nothing here to redo, and a re-run against an already-configured project can only make prod's credential and Vercel's disagree. Since #1607 `scripts/setup-demo-user.sh` **reuses `DEMO_USER_PASSWORD` when `.env.local` sets one** and mints a fresh one only when it does not, which is what lets `reset-db.sh` call it on every dev reset. That has a consequence on this path specifically: `.env.local` is pointed at prod here but still carries the *dev* password, so running the script as-is would set the prod demo user to the dev secret. **Comment out or clear `DEMO_USER_PASSWORD` in `.env.local` before running it below**, so prod gets a freshly minted secret of its own.
...
- [ ] The prod demo auth user exists, with `.env.local` still pointed at prod per the prerequisite above **and its `DEMO_USER_PASSWORD` cleared** per the block note — it prints a `DEMO_USER_EMAIL=` and a `DEMO_USER_PASSWORD=` line; keep both, the minted password is shown once and cannot be read back:
...
- [ ] `DEMO_USER_PASSWORD` set to the printed value as a Vercel **production** environment variable (unset, or disagreeing with what the demo user's password actually is, → `/demo` redirects to `/login?error=demo_unavailable`)
...
> PRE can reach the *policy* here since #1563 — `change-user.sh` now moves `profiles.user_id` alongside `barn_memberships.user_id`, so the storage RLS self-write check (keyed on `profiles.user_id`) passes under impersonation, and Phase 2-4 covers the path on your own profile. What PRE still cannot produce is the shape below: the writing uid belongs to a genuinely different Google account, on someone else's device, claiming a profile *you* created for them. There is no role branch in this path (`documents/new/page.tsx` gates on `isOwnPage`, `profile_photos_self_write` on `profiles.user_id`), so what's new here is a *different person's* claimed profile, not a different role. The photo steps below stay generic where PRE's equivalents name a `scripts/data/` file (#1135) — Casey Test is on their own device, with no repo checkout to pull a fixture from.
