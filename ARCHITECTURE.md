...
- **Comparison against barn data is barn-local, and so is an input default seeding a day of barn business.** (#1149/#1224) Anything deciding "is this date past / due / today" resolves today from `barns.timezone` via `barn-timezone.ts`'s `barnToday()`, computed server-side and passed to client components as a required prop — as do the new-expense Date and new-lease/boarding Start Date pre-fills (#1224; the day being seeded is the barn's business day, not the viewer's). `local-day.ts`'s `localToday()` is left with the defaults seeding the *viewer's own* scheduling choice — a new lesson's date (`DateHourPicker`), which month the lesson form's calendar opens on — plus decoding a stored instant back to an initial form value.
...
1. `/issueBatch create|pick|prune|defer` — maintain the release's candidate-issue file and pick what to work on next. `pick` fills every free worktree (detected via `scripts/workflow-context.sh`'s `worktree_state`) and reasons from the file's `note:`/`## Insights` prose rather than from a rule table (#1231); that prose is the file's real payload, and every skill that writes the file preserves it — this command's four subcommands, plus `/beginIssue` and `/finishIssue`, which move entries between sections (#1237)
