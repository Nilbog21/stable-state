...
| `notifications.ts` | Notification CRUD: `createNotification` (upserts on `user_id,barn_id,type`; resets `read_at` on conflict); `markNotificationRead` (sets `read_at = now()` by id); `markAllNotificationsRead` (sets `read_at = now()` for all unread for a user in a barn); `getNotifications(userId, barnId, limit=20)` (recent notifications ordered by `created_at DESC`) |
