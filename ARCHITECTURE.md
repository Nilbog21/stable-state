...
**The host's zone is fenced off.** `eslint.config.mjs` bans `toLocaleDateString`/`toLocaleTimeString`/`toLocaleString`, `Intl.DateTimeFormat`, and `Date`'s host-zone getters (`getHours`/`getMinutes`/`getSeconds`/`getDate`/`getMonth`/`getFullYear`/`getDay`) outside the date modules. A bare `new Date()` is deliberately *not* banned — comparing two instants or stamping `now` into a `TIMESTAMPTZ` is zone-free; the bug is always reading a calendar field off a Date. `scripts/ci.sh` runs the rule with `--max-warnings 0`, so it is what proves the conversion stayed complete.
