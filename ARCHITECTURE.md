...
| `trainer-documents.ts` | Trainer document CRUD: `getTrainerDocuments(trainerId, barnId)`, `createTrainerDocument(...)`, `deleteTrainerDocument(id, barnId)`, `getDocumentSignedUrl(storagePath)` — generates 300 s signed URL from the `documents` bucket |
| `rider-documents.ts` | Rider document CRUD: `getRiderDocuments(riderId, barnId)`, `createRiderDocument(...)`, `deleteRiderDocument(id, barnId)`, `getDocumentSignedUrl(storagePath)` — generates 300 s signed URL from the `documents` bucket |
