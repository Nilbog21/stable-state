...
| lesson_horses | SELECT, INSERT, UPDATE, DELETE | SELECT, INSERT, UPDATE | SELECT, INSERT |
| lesson_riders | SELECT, INSERT, UPDATE, DELETE | SELECT, INSERT, UPDATE | SELECT, INSERT |
...
| `/barn/[slug]/lessons/[id]` | All active members | Edit link visible to managers; trainer/manager see and can inline-edit `horse_notes` per horse and `rider_notes`+`private_notes` (visually distinguished) per rider via server actions; rider sees own `rider_notes` read-only |
...
| `/barn/[slug]/horses/[id]` | All active members | Horse detail page; shows horse name and current availability status; manager sees `HorseAvailabilityForm` to toggle `is_available` and set `unavailability_reason` (reason field disabled when available; submitting clears reason); trainer/rider see reason as read-only text when horse is unavailable |
