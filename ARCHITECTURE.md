...
Shared rule owners: `scripts/workflow-context.sh` (#1118 — worktree→port map, label→base-branch rule) and `scripts/select-specs.sh` (#1213 — which e2e specs a diff can break, via each spec's `// covers:` globs). Contracts for both: [`docs/scripts/workflow.md`](docs/scripts/workflow.md) and [`docs/scripts/suite.md`](docs/scripts/suite.md); `scripts/ci.sh` runs `select-specs.sh --lint`, which makes the declarations binding on fleet workers.
