...
Adjuncts: `/grillMe` (interview a plan or a work log's findings to shared understanding — the only path from a finding to a filed GitHub issue), `/backlogReview` (triage after a batch issue dump), `/estimateRelease` (velocity and cut planning), `/sync-migrations` (rename and push pending Supabase migrations), `/fableFleet` (run the main sequence above headlessly across a fleet of `fable-N` worktrees for a batch of small, related issues — the developer answers escalations instead of driving each skill; migration-bearing issues dispatch normally and serialize only on the orchestrator-held sync slot, #1293).
