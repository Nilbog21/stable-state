...
| `barn-memberships.ts` | Membership reads and writes; cross-barn user lookup (`getBarnMembershipsForUser`); `getActiveMembersWithProfiles(barnId, role)` — returns active members of a given role with names joined from `profiles`; `resolveMemberNames(membershipIds, client?)` — resolves a list of membership IDs to a `Map<membershipId, "First Last">` via two-query join to `profiles`, falling back to the raw membership ID when profile is missing |
