...
Adjuncts: `/grillMe` (interview a plan or a work log's findings to shared understanding — the only path from a finding to a filed GitHub issue), `/backlogReview` (triage after a batch issue dump), `/estimateRelease` (velocity and cut planning), `/sync-migrations` (rename and push pending Supabase migrations), `/fableFleet` (run the main sequence above headlessly across a fleet of `fable-N` worktrees — the developer answers escalations instead of driving each skill; migration-bearing issues dispatch normally and serialize only on the orchestrator-held sync slot, #1293). It has two modes: given a range or list of issue numbers it drains that batch, and given no arguments it runs the release backlog continuously (#1294), refilling worktrees from `/issueBatch pick` — which the **orchestrator** runs, not the workers, because `pick`'s central constraint is that no pick may block another *in the same set* and a worker picking alone has no set. Backlog mode also routes two decisions on what an artifact declares rather than on the orchestrator's read of an issue: a plan naming anything the acceptance criteria didn't escalates, and a PR adding a `(manual)` line to `PRE_RELEASE_TEST_CHECKLIST.md` parks for human verification.
