...
| `seeded_accounts` | `id UUID PK`, `email TEXT UNIQUE NOT NULL`, `first_name TEXT NOT NULL`, `last_name TEXT NOT NULL`, `barn_id UUID NOT NULL→barns`, `role TEXT NOT NULL→roles`, `created_at TIMESTAMPTZ NOT NULL DEFAULT now()` | Staging table for manager accounts seeded before first OAuth sign-in. A row is inserted by `createSeededAccount` (via `seed-account.ts`) and deleted by `activateSeededAccount` in the auth callback on first sign-in. RLS: service_role INSERT/DELETE (bypasses RLS by default); authenticated users SELECT own row (`email = auth.email()`). |
