...
| `/barn/[slug]/lessons/[id]/edit` | manager, trainer | Pre-filled edit form; instructor field read-only for trainers (DB enforces via RLS); group→normal downgrade shows warning and requires manager to select one rider/horse to keep; updates are atomic via `update_lesson_with_participants` RPC; only managers can add new horses or riders inline; `LessonNotesForm` renders below `LessonForm` for editing `horse_notes`/`rider_notes`/`private_notes` via `updateAllNotesAction` |
