...
`auth_can_read_instructor_membership(p_membership_id uuid, p_barn_id uuid)` is a `SECURITY DEFINER` SQL function that returns `true` if the calling user can already see (per `lessons` SELECT policies) a lesson whose `instructor_id` is `p_membership_id`. Used as the authorization check inside `get_instructor_membership_names` (see [`docs/architecture/rpc/members.md`](docs/architecture/rpc/members.md)) — SECURITY DEFINER avoids the same barn_memberships↔lessons recursion class a plain subquery would hit. Originally backed a same-named row-level RLS policy on `barn_memberships`, but that policy granted the full row (including `invite_token`, a live `claim_managed_member` bearer credential, since `barn_memberships` has no column-level GRANT restriction for `authenticated`) and was replaced by `get_instructor_membership_names` for that reason (#739 follow-up).

`auth_is_active_barn_member(p_barn_id uuid)` is a `SECURITY DEFINER` SQL function that returns `true` if the calling user has any active `barn_memberships` row in the given barn, regardless of role. Used inside `get_active_barn_member_summaries` (see [`docs/architecture/rpc/members.md`](docs/architecture/rpc/members.md)) rather than a new row-level SELECT policy on `barn_memberships` — a broad policy would satisfy #779's "any active member can browse the roster" requirement but would also expose `invite_token` on every row it authorizes, since `barn_memberships` has no column-level GRANT restriction for `authenticated` (same bug class the `auth_can_read_instructor_membership`/`get_instructor_membership_names` pair above already fixed once).
...
- **Global actions:** `src/app/actions/` — auth (`auth.ts`), lesson submission, payment-type update, stopping a recurring series, projected-exhaustion lookup, and month-range schedule read for the lesson and expense forms' conflict calendars (`lessons.ts` — it stays there rather than moving to `expenses.ts`, since the read itself is barn-wide and type-agnostic; #1020), whole-lesson and per-rider cancellation (`lesson-cancellation.ts`), notification mark-all-read (`notifications.ts`), expense create, update, delete, and recipient-type lookup (`expenses.ts`); `lesson-form-parsing.ts` — deliberately has no `'use server'` directive, so `parseLessonFormData` (shared parse/validate step for `submitLesson` and `updateLessonAction`, mirroring `parseExpenseFormData`) is never independently reachable as a Server Action and can't skip the `requireMembership` check its callers perform
...
**Times are barn times.** A lesson at 4:00 PM is 4:00 PM *at the barn* — for everyone, on every device, in every direction. #1222 deleted the viewer frame the app used to carry (#88, built out by #935/#955), leaving two frames:

- **A real instant is barn-local.** Every `TIMESTAMPTZ` that is rendered or compared travels as an `Instant` (`{ at, tz }`, `src/lib/db/types.ts`) — the value plus the barn it belongs to. Only the DAL mints one, so `format-date.ts`'s `formatBarnDateTime`/`formatBarnDate`/`formatBarnTime` take no timezone argument, offer no render-mode choice, behave identically in Server and Client Components, and cannot be called wrong. Comparisons ("is this past / due / today") resolve today from `barns.timezone` via `barn-timezone.ts`'s `barnToday()`, computed server-side and passed to client components as a required prop; date/hour *entry* anchors to the barn the same way, through `wallClockToInstant()`.

  Audit columns nobody displays (`submitted_at`, `updated_at`, `read_at`, and every `created_at` except `notifications.created_at`, which the notification dropdown does show) stay plain strings. The formatters accept nothing else, so displaying one later forces branding it at the DAL — which is the guarantee, not an oversight.

- **A calendar date is zoneless.** A `DATE` column (`agreement_charges.period`, `appointments.expense_date`, a document's `reminder_date`) and the "YYYY-MM-DD" strings the calendar views do arithmetic on name a day, not an instant. `local-day.ts` holds that arithmetic and `format-date.ts`'s `formatShortDate`/`formatShortDateOnly`/`formatChargePeriod` render it UTC-forced. Never pass an instant through those — #1223 branded this frame too, as `CalendarDate` (`src/lib/db/types.ts`), so the compiler enforces it: `formatShortDateOnly`/`formatChargePeriod` and every barn-day comparison take a `CalendarDate`, and a bare `string` or an `Instant` no longer typechecks into one. It needs no timezone argument, because a calendar day has no zone to carry.

  Three mints exist and no more, which is what makes "there is exactly one producer of today" true: `local-day.ts`'s `calendarDate` (unchecked, for a `DATE` column PostgREST returns as `string`) and `isValidDateString` (a type predicate — the validating mint for user input), plus `barn-timezone.ts`'s `barnDay`/`barnToday`, the one crossing from the instant frame into this one. Write inputs stay bare `string`: a form field is unvalidated text until something checks it. Per-module detail is in the module files under [`docs/architecture/dal/`](docs/architecture/dal.md).

**The host's zone is fenced off.** `eslint.config.mjs` bans `toLocaleDateString`/`toLocaleTimeString`/`toLocaleString`, `Intl.DateTimeFormat`, `Date`'s host-zone getters (`getHours`/`getMinutes`/`getSeconds`/`getDate`/`getMonth`/`getFullYear`/`getDay`), and the multi-argument `new Date(y, m, d, h, min)` constructor, outside the date modules. A bare `new Date()` — or `new Date(msOrIsoString)` — is deliberately *not* banned: comparing two instants or stamping `now` into a `TIMESTAMPTZ` is zone-free. The bug is always a calendar field crossing the host's clock, in either direction — the getters *read* one, the multi-arg constructor *writes* one. That second half was missed on the first pass and cost a real site: `ExpenseForm` went on anchoring expense entry to the viewer's zone after every other entry path had moved, because it reads no banned getter and the fence couldn't see it. `scripts/ci.sh` runs the rule with `--max-warnings 0`, so it is what proves the conversion stayed complete.

`/calendar.ics` is the one deliberate exception: it keeps emitting UTC `DTSTART`/`DTEND`, which is correct for the format — the subscriber's calendar client converts.
...
Adjuncts: `/grillMe` (interview a plan or a work log's findings to shared understanding — the only path from a finding to a filed GitHub issue), `/backlogReview` (triage after a batch issue dump), `/estimateRelease` (velocity and cut planning), `/sync-migrations` (rename and push pending Supabase migrations), `/fableFleet` (run the main sequence above headlessly across a fleet of `fable-N` worktrees — the developer answers escalations instead of driving each skill; migration-bearing issues dispatch normally and serialize only on the orchestrator-held sync slot, #1293). It has two modes: given a range or list of issue numbers it drains that batch, and given no arguments it runs the release backlog continuously (#1294), refilling worktrees from `/issueBatch pick` — which the **orchestrator** runs, not the workers, because `pick`'s central constraint is that no pick may block another *in the same set* and a worker picking alone has no set. In **both** modes, two decisions route on what an artifact declares rather than on the orchestrator's read of an issue — a plan naming anything the acceptance criteria didn't escalates, and a PR adding a `(manual)` line to `PRE_RELEASE_TEST_CHECKLIST.md` parks for human verification. Those are properties of the work, not of how the run was invoked; backlog mode is only what made them necessary, since arbitrary backlog work is where a bad design or an unverifiable flow ends in a merge rather than in a worker stopping to ask (#1294).

The five skills that detect a worktree — `/beginIssue`, `/reviewIssue`, `/testIssue`, `/finishIssue`, `/continueIssue` — open by running `scripts/workflow-context.sh` (#1118), which owns the worktree→port map and the label→base-branch rule and prints them as `key=value` lines — those two rules were previously restated in each skill, and had already diverged. See `scripts/CLAUDE.md` for the emitted keys. `/fableFleet`'s workers run those same five skills from a `fable-N` worktree, so that script resolves those too (port `3100+N`, by rule rather than by table entry) — an unresolved worktree makes every one of them stop and ask a user the worker doesn't have.

`scripts/select-specs.sh` (#1213) is the same shape for the same reason: it intersects a PR diff with the `// covers:` globs each `e2e/*.spec.ts` declares and prints `mode=full`/`mode=scoped`/`mode=none`, so `/testIssue`, `/beginIssue`, and `/reviewIssue` share one definition of which e2e specs a change can break instead of three divergent prose rules. Contract and the always-full list: `scripts/CLAUDE.md`. `scripts/ci.sh` runs its `--lint` pass, which is what makes the declarations binding on fleet workers.
