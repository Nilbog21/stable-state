...
| `horses.ts` | Horse registry; per-horse exertion summary (`getHorseExertionSummary`); `getHorsesByBarn` filters to active only; `getHorseById(horseId, barnId)` — single horse lookup, returns `null` if not found; `updateHorseDetails(horseId, barnId, updates)` — single-call update for `name?`, `is_active`, `is_available`, `unavailability_reason` (used by manager save form); sets `deactivated_at` to `now()` when `is_active` transitions to `false`, clears it to `NULL` on reactivation, leaves it untouched otherwise; `setHorseActive`, `setHorseAvailability` — individual field setters (still exported; used internally); `resolveHorseNames(horseIds, barnId, client?)` — resolves a list of horse IDs to a `Map<horseId, name>` scoped to `barnId`; callers fall back to raw horse ID when ID is absent from the map |
