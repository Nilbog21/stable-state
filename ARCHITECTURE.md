...
| `horses` | `id UUID PK`, `barn_id UUID NOT NULL→barns`, `name TEXT NOT NULL`, `is_active BOOLEAN NOT NULL DEFAULT true`, `created_at/updated_at TIMESTAMPTZ`; `UNIQUE(barn_id, id)` | `is_active=false` soft-deletes a horse; management queries filter to active, lesson history resolves horse names regardless |
