...
- **A real instant is barn-local.** Every `TIMESTAMPTZ` that is rendered or compared travels as an `Instant` (`{ at, tz }`, `src/lib/db/types.ts`) — the value plus the barn it belongs to. Only the DAL mints one, so `format-date.ts`'s `formatBarnDateTime`/`formatBarnDate`/`formatBarnTime` take no timezone argument, offer no render-mode choice, behave identically in Server and Client Components, and cannot be called wrong. Comparisons ("is this past / due / today") resolve today from `barns.timezone` via `barn-timezone.ts`'s `barnToday()`, computed server-side and passed to client components as a required prop; date/hour *entry* anchors to the barn the same way, through `wallClockToInstant()`.
  ...
  Audit columns nobody displays (`created_at`, `updated_at`, `submitted_at`, `read_at`) stay plain strings. The formatters accept nothing else, so displaying one later forces branding it at the DAL — which is the guarantee, not an oversight.
