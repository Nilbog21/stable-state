...
Called by `teardownBarnData` in `src/lib/db/service-role.ts` (relocated there from `scripts/script-utils.ts` by #505, which left a re-export behind — see `service-role.ts` in [`dal/service-role.md`](../dal/service-role.md)); `teardownBarnData`'s own callers are `/demo`'s reap-oldest-on-cap step, `/api/cron/reset-demo`'s reap loop, `e2e/support/fixtures.ts`, and `e2e/checklist-phase1-demo.spec.ts`'s stray-demo-barn `afterAll` cleanup (#1424).
