...
The token is checked against `barn_memberships.calendar_feed_token` entirely inside the `get_calendar_feed` `SECURITY DEFINER` RPC (see [`rpc.md`](../rpc.md)) — there is no `requireMembership` call here, since that helper's `redirect()`/`notFound()` calls are Server Component/Action-only and don't work inside a Route Handler; an unknown/inactive token → `404`.
