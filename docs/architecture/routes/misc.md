...
#506 — Route Handler (`src/app/api/cron/reset-demo/route.ts`), the other sanctioned exception to "no API routes" (see `/calendar.ics` above) — Vercel Cron (`vercel.json`'s `crons` entry, daily at 08:00 UTC — the Hobby plan's one-run-per-day limit, #1438) calls a plain HTTP endpoint and can't invoke a Server Action.
