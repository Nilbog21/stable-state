...
A valid token, when the visitor isn't yet signed in, redirects to `/barn/[slug]/login?token=...` to reuse the existing OAuth+claim flow (see `claim_managed_member` in [`rpc/members.md`](../rpc/members.md));
