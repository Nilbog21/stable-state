...
Since #1641 that confirmation also **names the account about to be bound** — "Signed in as {first} {last} ({email}) — not you? Sign out", falling back to the email alone when the session has no `profiles` row — because the token is single-use and this is the last point at which a claimant can see they are signed in as somebody else (a shared family device, a manager's own browser).
The demo case gets its own screen rather than that line: when the session's profile carries `is_demo` (the shared `/demo` account — see [`schema.md`](../schema.md)'s `profiles` row), the page renders "You're signed in as the demo account" plus **Sign out**, and no Accept button. It is caught at render rather than left to `claim_managed_member`'s `demo_account_cannot_claim` raise because that raise would round-trip through `acceptInvite`'s failure path to the "Invite invalid" screen — the worst possible message, since the invite is fine and the session is wrong. Both claim call sites translate that one rejection instead: `isDemoClaimRejection` (`member-invites.ts`) routes it back here with the token intact and **no** `?error`, so this page's own demo branch renders.
Both Sign out controls post to `register/actions.ts`'s `signOutAndReturnToInvite`, which signs out and redirects to `/barn/[slug]/login?token=...` rather than reusing `actions/auth.ts`'s `signOut` — that lands on `/login` and strands the claimant, who would have to find the emailed invite again.
