...
card shows date with the time appended only when one is set (no `—` placeholder — #947's own follow-up commits dropped the dashes the card first shipped with), recipient · expense type, horse(s) or "Entire Barn" (or `—` if none), and amount — a null amount renders "(no amount specified)" instead, turning amber alongside an amber "Past Due" badge once the expense is past due (`isExpensePastDue`, rewritten by #1481 to agree with `getOutstandingExpenses` on both halves it had disagreed on: the *set* is every **outstanding** expense — missing an amount, missing a payment type, or both — so an expense priced but never attributed to a payment method still badges, and the *frame* is the barn's own wall clock, `page.tsx` minting it once via `instantToLocalWallClock(new Date(), barn.timezone)` and threading it to every card (including through the `OlderExpensesToggle` client component, which must not read a clock of its own) rather than racing `Date.now()` against UTC-tagged digits; due datetime is `expense_date`+`expense_time`, or end-of-day when no time is set. Only the "(no amount specified)" text is amber — an entered amount stays zinc beside the badge);
