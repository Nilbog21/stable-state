...
(#1362) Every form in the `(protected)` group that holds user input arms that guard — no longer just the lesson form: client form components latch a local dirty flag (armed by bubbled form change, plus explicit arming for button-driven inputs) into `useUnsavedChangesGuard` (`NavigationBlocker.tsx` — context arm/clear plus `beforeunload` for hard navigations), and server-component pages' inline forms are wrapped in its sibling `GuardedForm`, which arms on change and clears on submit.
