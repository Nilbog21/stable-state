...
(#1362) Every form in the `(protected)` group that holds user input arms that guard — no longer just the lesson form: client form components latch a local dirty flag (armed by bubbled form change, plus explicit arming for button-driven inputs) into `useUnsavedChangesGuard` (`NavigationBlocker.tsx` — per-form arm/clear into the context's dirty *store* (form id → dialog message), so on pages mounting several guarded forms (`/settings`, `/members`) one form clearing can't disarm a still-dirty sibling, and the confirm dialog's message is derived from the store: the dirty forms' one distinct message when they agree, the generic default when they don't; plus `beforeunload` for hard navigations), and server-component pages' inline forms — the cancel and delete confirm pages included — are wrapped in its sibling `GuardedForm`, which arms on change and clears on submit.
Dirty state clears on successful save — in place for edit-style forms, by unmount after redirect for create-style forms — so a clean save never prompts.
Button-only confirm forms have no input to lose and stay unguarded; routes outside the group (`/login`, `/barn/[slug]/register`, `/profile`) are unguarded by construction, since the provider and `BlockingLink` chrome exist only in this layout.
