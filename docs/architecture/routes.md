...
(#1362) Every form in the `(protected)` group that holds user input arms that guard — no longer just the lesson form: client form components latch a local dirty flag (armed by bubbled form change, plus explicit arming for button-driven inputs) into `useUnsavedChangesGuard` (`NavigationBlocker.tsx` — per-form arm/clear into the context's dirty *set*, so on pages mounting several guarded forms (`/settings`, `/members`) one form clearing can't disarm a still-dirty sibling; plus `beforeunload` for hard navigations), and server-component pages' inline forms — the cancel and delete confirm pages included — are wrapped in its sibling `GuardedForm`, which arms on change and clears on submit.
