...
function netHeaderButton(page: Page) {
  return table(page).locator('thead button').filter({ hasText: 'Net' })
}

/**
 * Taps the Net header and waits for the indicator to land on the requested direction. The
 * wait is synchronisation, not a second assertion: sorting is client state, so a row read
 * taken straight after the click can race React's re-render.
 */
async function sortByNet(page: Page, expected: '▲' | '▼') {
  await netHeaderButton(page).click()
  await expect(netHeaderButton(page)).toContainText(expected)
}
...
test.describe.serial('by instructor sorting', () => {
  ...
  test('tapping_net_header_moves_the_ascending_indicator_to_net @manager', async ({ page }) => {
    await page.goto(financesUrl())
    await sortByNet(page, '▲')
    expect(await indicatedHeaders(page)).toEqual(['Net ▲'])
  })
