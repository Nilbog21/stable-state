...
**A patch's migrations are one file, not the usual split.** Where a feature splits schema / RPC / RLS across separate migration files (`ARCHITECTURE.md`'s Feature anatomy), a patch puts all of them in a single migration named `{timestamp}_{issue-number}_{short-description}.sql` — e.g. `20260819044709_1640_appointments_on_barn_calendar.sql`. A patch is one indivisible fix shipped on its own, so the parts have no separate life to justify separate files, and the issue number in the name is what ties the migration to the patch it belongs to when it is later picked up on the next release branch. Keep each part under its own `-- 1. Schema` / `-- 2. Functions` / `-- 3. Grants` banner inside the file. Feature migrations are unchanged — they stay split, and keep their descriptive names.
