...
**A line a PR adds is born automated or justified-manual.** Tag it either `(e2e: <test name>)`, with the covering spec written in that same PR, or `(manual)` with the reason stated on the line itself. `(e2e-candidate)` is not available for a line a PR *adds* — it defers automation indefinitely, which is how the manual pass grows monotonically with every feature. It remains correct for the *pre-existing* untagged lines an audit is converting (#1251 owns Phases 5–6): those verdicts are the case this rule preserves, not violations of it. The stated-reason requirement binds added lines the same way — the handful of older `(manual)` lines carrying no reason are grandfathered.

Legitimate `(manual)` grounds: a **human judgment call** — does this flow read well, cross-device look-and-feel, any visual or aesthetic check — or something needing a genuinely separate real person or prod configuration (which belongs in `POST_RELEASE_TEST_CHECKLIST.md` instead), or an external dependency a spec cannot drive. "Would take a while to automate" is not one.
