...
When a worker ends its turn, read the status block. Answer routine questions via SendMessage (the worker continues with context intact). When a worker finishes an issue, sweep its worktree's `specs/` per Step 6 — **before** the reassignment, which is what destroys the evidence — then assign the next unstarted issue from the batch to it.
