...
**Scope / size** — issues that are clearly too large for a single PR, independent of dependencies. Proposed action: suggest a split for reviewability — **by user-facing capability, not by layer**. Each resulting issue should still span the full file-touch sequence (schema → RLS → DAL → RPC → action → component → tests) as one vertical slice; a split into a schema issue plus an RLS issue plus a UI issue is the anti-pattern. Layer-only splits are fine when the layer is genuinely reusable across several unrelated future capabilities.
