...
**The pending marker.** Immediately before launching a full run, take `date +%s` and the current short HEAD, and write a line directly beneath `specs/issue-{N}.md`'s status marker:
```
<!-- pending_suite_run: launched {epoch} ({YYYY-MM-DD HH:MM}) — sha {short-sha} -->
```
...
**Only two steps may launch a suite run, and never two at once.** `run-checklist-suite.sh` truncates `checklist-suite.log` before it acquires its `e2e-slot.sh` slot, so a second launch while a run is in flight destroys the first run's evidence and then interleaves with it in the same file. Those two steps are this one and `/finishIssue` Step 2.5 — which is why `/reviewIssue`'s reconcile check leaves a `mode=full` verdict to this step instead of running it there, and why an inline minor fix below does not relaunch (Step 2.5 re-checks the diff since the marker's sha).
