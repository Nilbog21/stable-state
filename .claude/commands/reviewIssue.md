...
- **Agent 6:** Audit for project-specific conventions. Give this agent the `## Accepted deviations` list pulled from the work log above, with the instruction: these changes are already-approved and out of scope by design — do not flag them again.
  ...
  - **A new checklist line is born automated or justified-manual (CLAUDE.md's Pre-Release Checklist rule):** for every `PRE_RELEASE_TEST_CHECKLIST.md` line the diff *adds* — added lines only, never one it re-tags, reworks, or moves — add a finding at score 75 if the line carries `(e2e-candidate)`, carries `(e2e: <name>)` with no matching `test(...)` added under `e2e/` in the same diff, or carries `(manual)` with no reason on the line. The carve-out is load-bearing: an audit issue *converting* pre-existing untagged lines (#1251, Phases 5–6) is not adding them, so `(e2e-candidate)` there is correct and must not be flagged — check the `-` side of the hunk before deciding a line is new. A human-judgment reason (reads well, cross-device look-and-feel, a visual check) is a valid `(manual)` justification; only an absent or non-judgment reason ("hard to automate", "TODO", "will do later") is the finding.
  - **A UI diff with no checklist line at all:** if the diff touches `src/app/**` but adds no `PRE_RELEASE_TEST_CHECKLIST.md` line, add a finding at score 50: "This PR changes `src/app/**` but adds no checklist line — if the change is genuinely backend-only (a refactor with no user-visible difference), say so in the PR body; otherwise the checklist step was skipped." Score 50, not 75, because genuinely-backend-only and forgot-the-doc-step are otherwise indistinguishable from the diff alone. Suppress it entirely when the PR body or the issue already states the change is not user-visible.
