...
**Never start a review fan-out against a mutated or otherwise dirty tree.** Run `git -C {worktree-path} diff --quiet HEAD`; if it fails, refuse to launch the agents — print what `git status --short` shows and stop until the tree is clean (stash, commit, or revert — the user's call). The risk is a *silent* green review: a mutant can only mask a defect in the assertion it *replaces*, so a mutant that swaps an expected value for a wrong one makes the reviewer read a strictly **louder** signal than the truth — the noisy failure. Masking a real defect requires a mutant that makes an assertion WEAKER, and a weakening mutant that still passes is exactly what a survivor is. So the guard matters most when a mutation pass has survivors, least when it is clean — and since a review can't know which case it's in until it has already read the tree, it refuses the dirty tree outright.
...
- **Agent 6:** Audit for project-specific conventions. Give this agent the `## Accepted deviations` list pulled from the work log above, with the instruction: these changes are already-approved and out of scope by design — do not flag them again.
  ...
  - **A new checklist line is born automated or justified-manual (the born-automated rule in `PRE_RELEASE_TEST_CHECKLIST.md`'s header blockquotes, summarized in CLAUDE.md):** for every `checklists/pre-release/phase-*.md` *check* the diff adds, add a finding at score 75 if the line carries no automation tag at all, carries `(e2e-candidate)`, carries `(e2e: <name>)` with no matching `test(...)` added under `e2e/` in the same diff, or carries `(manual)` with no reason on the line **and no section-scoped reason covering it**. Since #1414 the reason may be stated once in a subsection's header blockquote instead of on each line, where every checkbox in that subsection shares one verdict and one rubric — the `Visual sweep` blocks are the case it exists for. Check the blockquote above a bare `(manual)` line before flagging it. The carve-out does not reach a one- or two-line block, which shares no rubric with anything: a bare `(manual)` there is still the finding. Ask whether the **check** is new, not whether the line's text is: an audit issue *converting* pre-existing untagged lines (#1251, Phases 5–6) adds no checks, so `(e2e-candidate)` there is correct and must not be flagged. A hunk's `-` side neither settles that nor is meant to — a hedge-closing PR (the hedge rule below) replaces a hedge line with the real check it was standing in for, which reads as an edit and *is* a new check, while a line moved between phases reads as new and is not. Valid `(manual)` reasons are exactly the index's list (`PRE_RELEASE_TEST_CHECKLIST.md`'s header blockquotes): a human judgment call (reads well, cross-device look-and-feel, a visual check), or an external dependency a spec cannot drive. An absent reason, or "hard to automate"/"TODO"/"will do later", is the finding — and so is a reason that clears a `POST_RELEASE_TEST_CHECKLIST.md` bar (needs a separate real person, prod config), which means the check belongs in POST rather than tagged `(manual)` here.
  - **A UI diff with no checklist line at all:** if the diff changes what a user sees or does — `src/app/**`, and shared UI under `src/components/**` too, since a `Button.tsx` change is user-visible everywhere it renders — but adds no `checklists/pre-release/phase-*.md` line, add a finding at score 50: "This PR changes user-facing code but adds no checklist line — if it needs none, say why in the PR body; otherwise the checklist step was skipped." Score 50, not 75, because a PR that legitimately needs no new line and one that forgot the doc step are indistinguishable from the diff alone. Suppress it entirely when the PR body or the issue already accounts for it — the change isn't user-visible, or an existing checklist line already covers the changed behavior, which is the ordinary case for a bug fix: the line asserting the correct behavior is already there.
...
- **Minor** (a self-contained tweak: copy, style, an off-by-one, updating one existing test's assertion): fix inline now:
  ...
  If the fix changed anything the e2e suite covers, pipe the diff through the selector before moving on — `/testIssue` Step 4 has the whole protocol (which command, backgrounding it, reading `checklist-suite.log`, the worktree port) and the rule that its verdict is **binding**, de-escalatable only by a recorded ruling. Follow both there; don't restate either here. Nothing to request and nobody to ask, on `/fableFleet` or anywhere else: `run-checklist-suite.sh` acquires its own slot from `scripts/e2e-slot.sh` (#1295) and simply waits if it is taken. Most reconcile fixes are copy or a single assertion and come back `mode=none`, which is the answer, not a reason to skip the check.

  **Run a `mode=scoped` selection here; leave a `mode=full` one to `/testIssue` Step 4.** That step re-derives the verdict from the whole PR diff minutes later and would reach the same answer, and it is always between here and a merge (`/finishIssue` Step 1 only trusts `status: ready`, which only `/testIssue` sets). Launching one here instead buys nothing and breaks something: `run-checklist-suite.sh` truncates `checklist-suite.log` before taking its slot, so the second of two runs in a worktree destroys the first's evidence — and since #1602 that log is what `/finishIssue`'s merge gate reads.
