...
- **Agent 6:** Audit for project-specific conventions. Give this agent the `## Accepted deviations` list pulled from the work log above, with the instruction: these changes are already-approved and out of scope by design — do not flag them again.
  ...
  - **A new checklist line is born automated or justified-manual (CLAUDE.md's Pre-Release Checklist rule):** for every `PRE_RELEASE_TEST_CHECKLIST.md` *check* the diff adds, add a finding at score 75 if the line carries no automation tag at all, carries `(e2e-candidate)`, carries `(e2e: <name>)` with no matching `test(...)` added under `e2e/` in the same diff, or carries `(manual)` with no reason on the line. Ask whether the **check** is new, not whether the line's text is: an audit issue *converting* pre-existing untagged lines (#1251, Phases 5–6) adds no checks, so `(e2e-candidate)` there is correct and must not be flagged. A hunk's `-` side neither settles that nor is meant to — a hedge-closing PR (the hedge rule below) replaces a hedge line with the real check it was standing in for, which reads as an edit and *is* a new check, while a line moved between phases reads as new and is not. Valid `(manual)` reasons are exactly CLAUDE.md's list: a human judgment call (reads well, cross-device look-and-feel, a visual check), or an external dependency a spec cannot drive. An absent reason, or "hard to automate"/"TODO"/"will do later", is the finding — and so is a reason that clears a `POST_RELEASE_TEST_CHECKLIST.md` bar (needs a separate real person, prod config), which means the check belongs in POST rather than tagged `(manual)` here.
  - **A UI diff with no checklist line at all:** if the diff changes what a user sees or does — `src/app/**`, and shared UI under `src/components/**` too, since a `Button.tsx` change is user-visible everywhere it renders — but adds no `PRE_RELEASE_TEST_CHECKLIST.md` line, add a finding at score 50: "This PR changes user-facing code but adds no checklist line — if it needs none, say why in the PR body; otherwise the checklist step was skipped." Score 50, not 75, because a PR that legitimately needs no new line and one that forgot the doc step are indistinguishable from the diff alone. Suppress it entirely when the PR body or the issue already accounts for it — the change isn't user-visible, or an existing checklist line already covers the changed behavior, which is the ordinary case for a bug fix: the line asserting the correct behavior is already there.
