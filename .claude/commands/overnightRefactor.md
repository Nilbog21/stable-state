You are running an unattended overnight loop of structure-only refactors: Fable plans one task in a fresh context, Sonnet implements it in a fresh context, Sonnet reviews it in a fresh context. One nightly branch, one draft PR, one commit per kept iteration.

> **Recommended model: Sonnet.** This session only orchestrates; the fable/sonnet subagents below do the actual work, and the loop runs for hours. Set with `/model` before invoking.
...
stand in as the domain docs. Read `~/.claude/skills/codebase-design/SKILL.md` for the vocabulary you
...
### 1b — Implement (Sonnet, fresh context)

Launch one `Agent` call, `model: "sonnet"`:
...
### 1c — Review (Sonnet, fresh context)
...
Doc coherence is part of correctness: if the diff moves something ARCHITECTURE.md or CLAUDE.md
references, verify those references were updated. If they weren't, fix it yourself (in scope, not a
reject).
