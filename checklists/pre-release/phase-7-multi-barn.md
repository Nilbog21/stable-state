...
- [ ] (manual — a local shell script run against the dev project; its success is the script's, not the app's) Create a second barn — completes successfully:
...
- [ ] (manual — an interactive shell script prompting for name and barn slug; asserts on its stdout, not on the app) Run `bash scripts/seed-account.sh`, accepting the default first/last name, and enter `test-barn-checklist` as the barn slug — creates a fresh managed-manager stub invite in that barn and prints `Invite path: /barn/test-barn-checklist/register?token=<uuid>`
