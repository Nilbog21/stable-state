...
> **The two-barn fixture every line below needs is `withSecondBarn`** (`e2e/support/test.ts`, #1415) — `withBarn`'s twin, called *after* it at module scope, keyed through `secondBarnKey` so neither barn's slug (nor the name `createBarn` derives from it) contains the other's, and torn down on its own `afterAll` so nothing leaks into the shared dev project. `checklist-phase7-cross-barn-isolation.spec.ts` and `checklist-phase7-multi-barn.spec.ts` both consume it unchanged; a new slice here should too rather than seeding a second barn by hand. The two remaining `(e2e-candidate)` lines below need the same thing. **The mobile lines further down need no separate Playwright project** — `checklist-phase7-multi-barn.spec.ts` runs the whole phase on `@manager` and sets the width on the context it holds, for the reasons its header note 3 records; `@mobile` would dispatch that file a second time and re-seed both barns plus its throwaway login.
