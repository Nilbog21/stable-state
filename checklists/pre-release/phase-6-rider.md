...
- [ ] (e2e: rider_without_a_document_privilege_sees_no_documents_section) (#999) On a horse Dana neither owns nor holds a document grant on, no Documents section appears for her at all
- [ ] (e2e: rider_owned_horse_shows_the_documents_section) (#1547) On a horse Dana **owns**, her Documents section appears with no Access grant behind it at all — ownership alone admits her. Walking the dev barn, this can only be read on Clover, whose Set-as-Owner setup above also left her a `'write'` grant; an e2e run seeds a fourth horse she owns with no privileges row, which is the only fixture that isolates ownership from the grant
- [ ] (e2e: rider_owned_horse_shows_the_add_document_button) (#1547) That horse's Documents section shows the **Add Document** button — an owner scores `'write'`, the top of the ladder
- [ ] (e2e: rider_owned_horse_document_delete_removes_its_row) (#1547) That section's document rows carry a **Delete** button for Dana, and using it removes the row. This is the half a manager-granted `'write'` never confers, so it isolates ownership on Clover too
- [ ] (e2e: rider_owned_horse_shows_a_collapsed_upcoming_lessons_section) (#1547) That same owned horse's page shows a collapsed **Upcoming Lessons** section listing its lessons — ownership carries lesson-read as well, likewise without a grant
