...
- [ ] (e2e: an_off_hours_expense_time_warns_that_the_am_pm_may_be_inverted) (#1646) Set that form's **Time** to 6:30 AM — an amber line under the field reads `Check AM/PM — this is 6:30 AM, not 6:30 PM.` Nothing appears while the optional Time is still empty
...
- [ ] (e2e: the_access_tables_controls_all_submit_through_enhanced_forms) (#1390/#1549) On Apple's detail page, expand **Access** — every control there (Grant Access, the Owner radio, the three Documents radios, the Lesson Schedule switch, Revoke) is a real form button rather than a script-only one, so none is a dead click in the moment before the page finishes loading. The radios in particular are `<button type="submit" role="radio">`, not `<input type="radio">`, which would need JS to commit. Verified against the HTML the server sends, not by clicking
...
- [ ] (e2e: a_document_access_choice_survives_a_reload) (#1390) In a grant row's **Documents** radio group, tap a state other than the one already selected → reload the page → your choice is the selected one. #1390's testing round found the `<select>` this replaced leaving the chosen, stored and displayed values all disagreeing
- [ ] (e2e: a_manager_saving_notes_on_a_horse_they_do_not_own_is_not_rejected) (#1390/#1549) Saving **Feed & Medication** on a horse owned by **somebody else** (Apple, owned by a rider) succeeds as manager, with no error above the form — this path goes through `update_horse_notes`, which admitted the owning member alone until #1390. The unowned-horse case this line used to name stopped existing when #1549 made the column NOT NULL
...
- [ ] (e2e: setting_yourself_as_owner_puts_my_horses_at_the_top_of_the_horses_list) (#1000/#1549) Make yourself the owning member of Apple (expand **Access**; it is seeded rider-owned) → **My Owned Horses** is the first section on the Horses list
- [ ] (e2e: owned_horse_appears_under_my_horses) (#1000) Apple appears under **My Owned Horses**
- [ ] (e2e: owned_horse_shows_a_green_active_badge_under_my_horses) (#1000) Apple shows a green **Active** badge there
- [ ] (e2e: owned_horse_no_longer_appears_under_available) (#1000) Apple no longer appears under Available
- [ ] (e2e: owned_horse_card_shows_an_exhaustion_bar) (#1391) Apple's card under **My Owned Horses** also shows an exhaustion bar — owned horses used to be skipped by the exhaustion fetch entirely
...
- [ ] (e2e: removing_a_member_who_still_owns_horses_is_refused_naming_them) (#1549) Setup — make Indigo Test the owning member of a horse (grant them Access on Eclipse, then tap **Set as Owner**). Back on their member page, tap **Remove** and confirm → the removal is refused and a message under the button names Eclipse. Hand Eclipse back to yourself before continuing
- [ ] (e2e: confirming_the_remove_prompt_redirects_to_the_members_list) Tap **Remove** and confirm the browser prompt → you're redirected to the Members list
