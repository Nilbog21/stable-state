...
- [ ] (e2e: the_terms_page_table_of_contents_links_to_each_heading) The `/terms` page's **Contents** list names every section and clicking an entry scrolls to it
...
Both pages read that file at request time and render it through `<MarkdownDocument>` (`src/app/privacy/page.tsx:14,30`), which is `<ReactMarkdown>` plus the generated contents list (#1556). Deliberately unscoped: the line asks for a review and you decide how deep it needs to go.
...
**Seeded baseline after reset** (expect this data alongside anything you create below): trainers Alex, Blake, Casey; riders Dana, Emery, Finley (Emery has a landscape profile photo, Finley the portrait one added in #1639); second manager Morgan Manager; horses Apple, Butter, Clover; horse Willow (retired/inactive with 3 past lessons + 1 upcoming — will not appear in the horse picker or the Horses page's Available/Unavailable sections, only visible to managers under Inactive); horse Hazel (available-flag off, reason "Recovering from minor injury", feed notes but no medication notes — the Horses page lists it under Unavailable; since #1639 it also carries the **portrait** photo, the one seeded photo taller than it is wide, so the phone-width photo checks have a subject without needing an upload); horse Juniper (#1413's calendar-band fixture: exhaustion thresholds lowered to 3/8 and four upcoming lessons, so the New Lesson form's month calendar has an amber day, a red day and a tinted next-month day to compare); tiers Normal Tier ($100, default) and Premium Tier ($150); ~38 lessons spread over the past 3 months (some paid, one group per five, some jumping, 5 upcoming); two horse documents (#1559) — a coggins on Butter whose reminder date is 14 days in the past, so it shows a due badge on the horse's Documents section and is the row the manager dashboard's Document Reminders card lists, and a shot record on Apple with no reminder set, reachable on the owner path since Apple is Dana's horse.
