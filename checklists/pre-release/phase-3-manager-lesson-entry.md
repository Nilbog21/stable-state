...
- [ ] (e2e: the_new_lesson_form_opens_with_an_empty_start_time_and_no_exhaustion_bars) The **New Lesson** form *opens* with **Start Time** empty — so no lesson instant is selected — and no exhaustion bars render; entering a time brings them in. (Not "before a date is picked": since #1019 the calendar opens on today, so a day is always selected. Since #1578 the start time is what is unset on arrival.)
