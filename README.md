...
| `DEMO_USER_EMAIL` | `/demo` + demo setup script | Address of the shared auth user every demo visitor signs in as, created by `setup-demo-user.sh`. `/demo` returns a 404 when it is unset, so the public demo flow is off by default on a project that has not been through that setup step |
...
| `DEMO_BARN_CAP` | `/demo` + `/api/cron/reset-demo` (optional) | Maximum number of demo barns allowed to exist at once before `/demo` reaps the oldest to make room; the nightly cron route reaps down to the same cap. Defaults to `20` when unset; `0` means unlimited |
