-- Fix "UNNEST() with multiple arguments cannot have a column definition list".
-- PostgreSQL infers column types from the array types; explicit type annotations are invalid here.
DROP FUNCTION create_lesson_with_participants(uuid, uuid, timestamptz, numeric, uuid[], integer[], uuid[], lesson_type, boolean);
...
BEGIN
  ...
  INSERT INTO lessons (barn_id, instructor_id, lesson_at, fee, lesson_type, jumping)
  VALUES (p_barn_id, p_instructor_id, p_lesson_at, p_fee, p_lesson_type, p_jumping)
...
GRANT EXECUTE ON FUNCTION create_lesson_with_participants(uuid, uuid, timestamptz, numeric, uuid[], integer[], uuid[], lesson_type, boolean) TO authenticated;
