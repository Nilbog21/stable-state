...
async function run() {
  ...
  console.log(`  Riders:   ${DEV_RIDERS.map((r) => r.email).join(', ')} (${DEV_RIDERS[1].firstName} has a landscape profile photo, ${DEV_RIDERS[2].firstName} the portrait one)`)
  console.log(`  Horses:   ${DEV_HORSES[0]}, ${DEV_HORSES[1]} (photo set), ${DEV_HORSES[2]}, plus ${DEV_RETIRED_HORSE} (retired, deactivated_at 30 days ago, 3 past lessons + 1 upcoming), plus ${DEV_UNAVAILABLE_HORSE} (unavailable: "${DEV_UNAVAILABLE_REASON}", portrait photo set), plus ${DEV_CALENDAR_BAND_HORSE} (thresholds ${DEV_CALENDAR_BAND_THRESHOLDS.moderate}/${DEV_CALENDAR_BAND_THRESHOLDS.high} + 4 lessons: day +${DEV_CALENDAR_BAND_MODERATE_DAY_OFFSET}, day +${DEV_CALENDAR_BAND_HIGH_DAY_OFFSET}, and the ${DEV_CALENDAR_BAND_NEXT_MONTH_DAY}rd of next month — so its New Lesson month calendar always shows an amber day, a red day, and a tinted neighbouring-month day for #1413's manual dark-mode checks)`)
  console.log(`  Horse owners (#1549 — every horse has one): ${DEV_HORSES[0]} → ${DEV_RIDERS[0].firstName} ${DEV_RIDERS[0].lastName} (rider); ${DEV_CALENDAR_BAND_HORSE} → ${DEV_TRAINERS[0].firstName} ${DEV_TRAINERS[0].lastName} (trainer); the rest → ${DEV_MANAGER_2.firstName} ${DEV_MANAGER_2.lastName} (manager), which is what the Add Horse form assigns`)
