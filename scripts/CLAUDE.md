...
Raw `supabase.from(...)` calls are used only when no db layer equivalent exists (e.g. barn insert, active membership insert, teardown deletes, `auth.admin.*` calls), or when the equivalent is RPC-backed with an auth check that blocks service-role callers (`auth.uid()` is null under a service-role client) — e.g. `createManagedMember` → `create_managed_member`'s `auth_is_barn_manager` check; see `seed-account.ts`. Do not "fix" such scripts by switching them to the DAL function — they will fail at runtime: the class's known members (`create_managed_member`, `create_or_update_notification`, `collect_lesson_payment`) each raise `not_authorized` from an in-body check that resolves false on a null `auth.uid()`. Before #1546 the call died earlier still, at the ACL — these three had no `service_role` `EXECUTE`; the blanket grant moved the failure later without changing the guidance.
