...
| `seed-account` | ✓ | ✓ | ✓ | — | Creates a managed-manager stub (direct service-role inserts) and prints the invite path; `.test.ts` covers `buildInvitePath`; `--allow-prod` flag bypasses the `assertDevProject` dev-project check for the documented one-time production bootstrap use (README's "Production bootstrap" step 2) — mirrors the `--skip-env-local-check` flag pattern used by `seed-test-barn`/`teardown-test-barn` |
