...
>
> - `(e2e: <test name>)` — covered by that Playwright test in `e2e/`; run via `scripts/run-checklist-suite.sh`. **Machine-checked** (#1386): `scripts/check-e2e-tags.sh`, wired into `ci.sh`, fails the PR if the named test doesn't exist in `e2e/*.spec.ts` or carries no `@project` tag — either one silently launders an unverified checkbox into a green run. The tag is the test's title with its trailing `@project` suffixes dropped
