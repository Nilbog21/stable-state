...
> - **`(e2e-candidate)` items** may bundle a chained sequence of assertions into one checkbox when that avoids duplicate setup (#1010). Once automated, the resulting e2e spec still asserts each step individually — the per-step diagnosability this Convention protects just moves into the spec instead of the checklist. `(manual)` items don't get this exception and must still keep one assertion per checkbox.
>
...
- [ ] (#1006) As manager, grant this trainer a horse-privileges row on a horse (Access section) then make them that horse's owning member; reopen the horse as this trainer — **Feed Notes**/**Medication Notes** are now editable textareas with a **Save** button; edit and save both, then reload and confirm the new text persists
...
- [ ] (#1006) As manager, make Dana the owning member of a horse she has no privileges on yet (Access section); reopen that horse as Dana — **Feed Notes**/**Medication Notes** are editable textareas with a **Save** button; on a horse Dana does *not* own, they remain read-only text
