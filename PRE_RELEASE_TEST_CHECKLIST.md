...
>
> **Suite-ordering markings** (#1561) — a `(manual)` line whose step conflicts with a suite run carries one of `before-suite` or `after-suite` inside its parens, ahead of the reason: `(manual, before-suite — …)`. `before-suite` is a step that wipes or reseeds the shared dev project, which destroys the suite's own fixtures if it runs alongside one; `after-suite` is a step that restarts or replaces the server under test, which kills a suite driving that origin. Neither appears without `manual` — an `(e2e:)` line *is* the suite. `/runChecklist` greps for these to order its run, so the markings are the source of truth and there is no list of lines anywhere else to keep in sync. Everything else is safe to walk while the suite runs.
>
...
- [ ] The shared demo user exists in the target Supabase project — `bash scripts/setup-demo-user.sh`, with both the `DEMO_USER_EMAIL` and `DEMO_USER_PASSWORD` lines it prints pasted into `.env.local` (`/demo` 404s without the email and bails without the password). Since #1607 `scripts/reset-db.sh` re-runs it for you and `setup-demo-user` reuses the `DEMO_USER_PASSWORD` already in `.env.local`, so a reset no longer invalidates these two values — before that it did, and Phase 1's `/demo` checks failed with a `?error=demo_unavailable` redirect naming nothing
