...
- [ ] Open that invite path and click **Sign in with Google** as `DEV_EMAIL` (already signed in elsewhere in this browser — Google may bounce back immediately or show a full re-auth/consent screen depending on the OAuth app's session state in Google Cloud Console, which this repo doesn't control; either is expected) → claim succeeds and you land in **test-barn-checklist** as manager — no `?error=invite_claim_failed` redirect
