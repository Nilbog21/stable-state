...
- [ ] **By Instructor** tab: collected income per trainer full name (net of the instructor cut), plus a **Raw Fees** column showing that trainer's pre-cut lesson fees; trainer name is an underlined link to a new drill-down `/barn/dev-barn/finances/trainers/[id]` — one table of that trainer's paid lessons (date linking to the lesson, Type always "Lesson", fee net of the cut), bottom **Total** matches the By Instructor summary's Income figure; month param preserved
