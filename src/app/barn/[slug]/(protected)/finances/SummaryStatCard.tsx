...
export function SummaryStatCard({ label, value, infoText }: { label: string; value: string; infoText: string }) {
  ...
  return (
    <button
      ...
      onClick={() => setOpen((o) => !o)}
      aria-expanded={open}
      className={`${cardBaseClass} relative w-full bg-white p-3 text-left dark:bg-zinc-900`}
    >
      <section>
      ...
      {open && (
        <span className="absolute left-3 top-full z-10 mt-1 w-48 rounded-md border border-zinc-200 bg-white p-2 text-xs font-normal normal-case tracking-normal text-zinc-700 shadow-md dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
          {infoText}
        </span>
      )}
    </button>
