...
describe('useSortableRows', () => {
  it('should_sort_by_default_key_ascending_on_mount', () => {
    const { result } = renderHook(() => useSortableRows<Row, 'name' | 'value'>(rows, getValue, 'name'))
  ...
  it('should_flip_to_descending_when_clicking_the_already_active_column', () => {
    const { result } = renderHook(() => useSortableRows<Row, 'name' | 'value'>(rows, getValue, 'name'))
  ...
  it('should_flip_back_to_ascending_on_a_third_click_of_the_same_column', () => {
    const { result } = renderHook(() => useSortableRows<Row, 'name' | 'value'>(rows, getValue, 'name'))
    act(() => result.current.toggleSort('name'))
    act(() => result.current.toggleSort('name'))
    expect(result.current.sortDir).toBe('asc')
  ...
  it('should_reverse_row_order_when_a_numeric_column_flips_to_descending', () => {
    const { result } = renderHook(() => useSortableRows<Row, 'name' | 'value'>(rows, getValue, 'name'))
    act(() => result.current.toggleSort('value'))
    act(() => result.current.toggleSort('value'))
