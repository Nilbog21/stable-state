...
describe('getLessonById', () => {
  ...
  it('should_call_resolve_member_names_with_only_rider_membership_ids', async () => {
    ...
    await getLessonById('lesson-1', 'barn-1', 'trainer', 'America/New_York')
  ...
  it('should_resolve_instructor_user_id_via_get_membership_by_id_for_barn_even_for_rider_role', async () => {
    ...
    const result = await getLessonById('lesson-1', 'barn-1', 'rider', 'America/New_York')
  ...
  it('should_call_get_membership_by_id_for_barn_with_instructor_id_and_barn_id', async () => {
    ...
    await getLessonById('lesson-1', 'barn-1', 'rider', 'America/New_York')
  ...
  it('should_propagate_error_when_resolve_member_names_rejects', async () => {
    ...
    await expect(getLessonById('lesson-1', 'barn-1', 'trainer', 'America/New_York')).rejects.toThrow('member names error')
  })
  ...
  // #1286: both embeds are to-many, and PostgREST returns a to-many embed's rows in
  // unspecified order. Sorted here rather than via .order(..., { referencedTable }) because
  // a rider's name lives in `profiles`, which this query's embed never reaches — it is
  // resolved by resolveMemberNames afterwards, so only a post-fetch sort can order both
  // lists the same way. Alphabetical, matching getHorsesByBarn's ORDER BY h.name.
  it('should_order_lesson_horses_alphabetically_by_name', async () => {
    const lessonData = {
      ...createMockLesson({ instructor_id: null }),
      lesson_horses: [
        { horse_notes: null, horses: { id: 'horse-z', name: 'Zephyr' } },
        { horse_notes: null, horses: { id: 'horse-a', name: 'Apollo' } },
      ],
      lesson_riders: [],
    }
    ...
    expect(result?.lesson_horses.map((lh) => lh.horses?.name)).toEqual(['Apollo', 'Zephyr'])
  })

  it('should_sort_a_lesson_horse_with_no_readable_horse_row_before_named_ones', async () => {
    // `horses` comes back null when the row is filtered out by the horses RLS policy.
    // Such a row sorts on an empty name rather than throwing.
    const lessonData = {
      ...createMockLesson({ instructor_id: null }),
      lesson_horses: [
        { horse_notes: null, horses: { id: 'horse-a', name: 'Apollo' } },
        { horse_notes: null, horses: null },
      ],
      lesson_riders: [],
    }
    mockLessonsFrom(lessonData)
  ...
  it('should_sort_an_unresolved_rider_by_the_membership_id_it_falls_back_to', async () => {
    const lessonData = {
      ...
      lesson_horses: [],
      lesson_riders: [
        { rider_id: 'mem-unresolved', barn_memberships: { user_id: null } },
        { rider_id: 'mem-1', barn_memberships: { user_id: null } },
      ],
    }
    mockLessonsFrom(lessonData)
    vi.mocked(resolveMemberNames).mockResolvedValue(new Map([['mem-1', 'Ada Rider']]))

    const result = await getLessonById('lesson-1', 'barn-1', 'trainer', 'America/New_York')

    expect(result?.lesson_riders.map((lr) => lr.barn_membership?.name)).toEqual(['Ada Rider', 'mem-unresolved'])
  })
  ...
  it('should_return_null_when_lesson_not_found', async () => {
    ...
    const result = await getLessonById('nonexistent', 'barn-1', 'trainer', 'America/New_York')
  ...
  it('should_throw_when_supabase_returns_an_error', async () => {
    ...
    await expect(getLessonById('lesson-1', 'barn-1', 'trainer', 'America/New_York')).rejects.toThrow('db error')
  })
  ...
  it('should_not_select_private_notes_for_any_role', async () => {
    ...
    await getLessonById('lesson-1', 'barn-1', 'manager', 'America/New_York')
  ...
  it('should_throw_when_get_lesson_horse_exertion_levels_rpc_errors', async () => {
    ...
    await expect(getLessonById('lesson-1', 'barn-1', 'trainer', 'America/New_York')).rejects.toThrow('exertion rpc error')
  })
  ...
  it('should_merge_rider_notes_from_rpc_onto_matching_rider', async () => {
    ...
    const result = await getLessonById('lesson-1', 'barn-1', 'manager', 'America/New_York')
  ...
  it('should_throw_when_get_lesson_rider_notes_rpc_errors', async () => {
    ...
    await expect(getLessonById('lesson-1', 'barn-1', 'manager', 'America/New_York')).rejects.toThrow('rider notes rpc error')
  })
  ...
  it('should_not_call_the_payment_info_rpc_when_lesson_is_not_found', async () => {
    ...
    await getLessonById('nonexistent', 'barn-1', 'trainer', 'America/New_York')
